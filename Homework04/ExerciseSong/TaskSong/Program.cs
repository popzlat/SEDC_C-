...
namespace TaskSong
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Jerry's favorite songs are:");
            Jerry.GetFavoriteSong();
            Console.WriteLine("");

            Console.WriteLine("Maria's favorite songs are:");
            Maria.FavoriteSongs = Songs.Where(song => song.Length >= 360).ToList();
            Maria.GetFavoriteSong();
            Console.WriteLine("");
            
            Console.WriteLine("Jane's favorite songs are:");
            Jane.FavoriteSongs = Songs.Where(x => x.Genre == Genre.Rock).ToList();
            Jane.GetFavoriteSong();
            Console.WriteLine("");

            Console.WriteLine("Stefan's favorite songs are:");
            Stefan.FavoriteSongs = Songs.Where(x => x.Genre == Genre.Hop_Hop).Where(x => x.Length <= 180).ToList();
            Stefan.GetFavoriteSong();
            Console.WriteLine("");

            var personsWithFourSongs = FansArray.Where(x => x.FavoriteSongs.Count >= 4).ToList();
            Console.WriteLine("Persons with four or more songs are : ");
            foreach (var item in personsWithFourSongs)
            {
